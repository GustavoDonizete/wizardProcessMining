...
def ler_cabecalho(caminho: str) -> tuple[list[str], dict]:
    """
    Lê apenas o cabeçalho e conta linhas do arquivo sem carregar tudo na memória.
    Retorna (lista_de_colunas, {"rows": int, "cols": int}).
    Usado no step1 para ir direto ao mapeamento enquanto o Parquet é gerado em background.
    """
    ...
    if ext == ".parquet":
        ...
        return cols, {"rows": rows, "cols": len(cols)}

    if ext == ".csv":
        params      = _detectar_csv_params(caminho)
        # Lê só o cabeçalho
        df_head     = pd.read_csv(caminho, nrows=0, **params)
        colunas     = list(df_head.columns)
        # Conta linhas sem carregar — lê em chunks só para somar
        rows = sum(1 for chunk in pd.read_csv(
            caminho, chunksize=CHUNK_SIZE,
            usecols=[colunas[0]], **params
        ) for _ in [chunk])
        return colunas, {"rows": rows, "cols": len(colunas)}

    if ext in (".xlsx", ".xls"):
        df_head = pd.read_excel(caminho, nrows=5)
        colunas = list(df_head.columns)
        # Para Excel conta via openpyxl sem carregar dados
        try:
            from openpyxl import load_workbook
            wb   = load_workbook(caminho, read_only=True, data_only=True)
            ws   = wb.active
            rows = ws.max_row - 1  # desconta cabeçalho
            wb.close()
        except Exception:
            rows = 0
        return colunas, {"rows": rows, "cols": len(colunas)}
    ...
    if ext == ".json":
        df_head = pd.read_json(caminho, nrows=5) if ext == ".json" else pd.read_json(caminho)
        colunas = list(df_head.columns)
        return colunas, {"rows": 0, "cols": len(colunas)}
...
def _transformar_fonte(source: dict) -> pd.DataFrame:
    ...
    for i in range(0, len(df_full), CHUNK_SIZE):
        ...
        if col_ts_end:
            parte["Timestamp_End"] = ts_end.values
    ...
    if not partes:
        base_cols = ["Case_ID", "Activity", "Timestamp_Start", "Source"]
        if col_ts_end:
            base_cols.insert(3, "Timestamp_End")
        return pd.DataFrame(columns=base_cols + cols_extras)
...
def construir_event_log(sources: list) -> tuple[pd.DataFrame, list]:
    """
    Empilha todas as fontes em chunks.
    Timestamp_End só aparece na saída se pelo menos uma fonte tiver mapeado.
    Retorna (DataFrame, erros).
    """
    partes, erros = [], []
    for s in sources:
        try:
    ...
    alguma_tem_ts_end = any("Timestamp_End" in p.columns for p in partes)

    if alguma_tem_ts_end:
        # Garante que todas as partes tenham a coluna (NaT onde não existe)
        for i, p in enumerate(partes):
            if "Timestamp_End" not in p.columns:
                partes[i] = p.copy()
                partes[i].insert(
                    p.columns.get_loc("Source"),
                    "Timestamp_End",
                    pd.NaT
                )
