...
def step1_post():
    ...
    try:
        # Lê apenas o cabeçalho e shape para ir direto ao mapeamento
        colunas, shape = core.ler_cabecalho(caminho)
        monitor.log_file(_sid(), nome, caminho, shape["rows"], shape["cols"])
    except Exception as e:
        monitor.log_error(_sid(), "step1", nome, str(e))
        flash(f"Erro ao ler arquivo: {e}", "danger")
        return redirect(url_for("step1"))

    # Conversão para Parquet em background — estará pronta quando chegar no step3
    threading.Thread(
        target=core.converter_para_parquet,
        args=(caminho,),
        daemon=True
    ).start()
