...
def step1_post():
    ...
    estado["pending"] = {
        ...
        "mapeamento":  {},   # { col: { "tipo": str, "incluir": bool } }
    }
...
def step2_post():
    ...
    obrigatorios_faltando = core.TIPOS_OBRIGATORIOS - tipos_mapeados
    if obrigatorios_faltando:
        flash(f"Mapeamento incompleto. Faltam: {', '.join(obrigatorios_faltando)}.", "warning")
        return redirect(url_for("step2"))
...
def step3():
    ...
    col_ativ = next(
        (c for c, v in p["mapeamento"].items() if v["tipo"] == "Atividade"), None
    )
    if not col_ativ:
...
def step5():
    ...
    # Monta lista de colunas disponíveis para criptografia por fonte
    # (todas as colunas mapeadas de cada fonte)
    fontes_colunas = []
    for s in estado["sources"]:
        fontes_colunas.append({
            "source_name": s["source_name"],
            "colunas": list(s["mapeamento"].keys()),
        })

    # Também oferece as colunas do Event Log consolidado (padrão)
    colunas_event_log = ["Case_ID", "Activity", "Timestamp_Start", "Timestamp_End", "Source"]
    # Adiciona colunas extras que foram incluídas
    for s in estado["sources"]:
    ...
    return render_template(
        "step5.html",
        step=5,
        colunas_event_log=colunas_event_log,
    )

@app.route("/step5", methods=["POST"])
def step5_post():
    estado          = _estado()
    colunas_cripto  = request.form.getlist("colunas_cripto")
    estado["colunas_cripto"] = colunas_cripto
...
def download_csv():
    ...
    if not path or not Path(path).exists():
        flash("Arquivo CSV não encontrado.", "warning")
        return redirect(url_for("step6"))
...
def download_sql():
    ...
    if not path or not Path(path).exists():
        flash("Arquivo SQL não encontrado.", "warning")
        return redirect(url_for("step6"))
...
def download_depara():
    ...
    if not path or not Path(path).exists():
        flash("Arquivo de-para não encontrado.", "warning")
        return redirect(url_for("step6"))
    return send_file(
